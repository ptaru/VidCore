...
public final class VTDecoder: @unchecked Sendable {
    ...
    private func processDolbyVisionConfig(_ doviConfig: Data) {
        ...
        doviConfig.withUnsafeBytes { ptr in
            ...
            print("[VTDecoder] Dolby Vision Profile \(dvProfile) Level \(dvLevel) detected (version \(dvVersionMajor).\(dvVersionMinor))")
            print("[VTDecoder] Flags: rpu=\(rpuPresentFlag) el=\(elPresentFlag) bl=\(blPresentFlag) compat_id=\(dvBlSignalCompatibilityId)")
            ...
            dvcC[3] = ((dvLevel & 0x1F) << 3) | ((rpuPresentFlag & 0x01) << 2) |
                      ((elPresentFlag & 0x01) << 1) | (blPresentFlag & 0x01)
    ...
    private func createFormatDescription(config: VTDecoderConfig) throws {
        ...
        if config.colorPrimaries == 9 { // BT.2020
            extensions[kCVImageBufferColorPrimariesKey as String] = kCVImageBufferColorPrimaries_ITU_R_2020
        } else if config.colorPrimaries == 1 { // BT.709
            extensions[kCVImageBufferColorPrimariesKey as String] = kCVImageBufferColorPrimaries_ITU_R_709_2
        }
    ...
    public func decodePacketSync(data: Data, pts: Int64, dts: Int64, duration: Int64) throws -> CVPixelBuffer? {
