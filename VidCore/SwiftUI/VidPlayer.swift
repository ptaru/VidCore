    ...
    public var body: some View {
        ...
        ZStack {
            ...
            if let videoInfo = activePlayer.videoInfo {
                ...
                .aspectRatio(
                    CGSize(width: videoInfo.width, height: videoInfo.height), contentMode: .fit)
            } else {
                SubtitleView(subtitle: activePlayer.currentSubtitle)
            }
            // User overlay
...
extension VidPlayer where Overlay == EmptyView {
    ...
    public init(
        player: MediaPlayer, showsBuiltInControls: Bool = true, allowsDebugMenu: Bool = false
    ) {
        self.init(
            player: player, showsBuiltInControls: showsBuiltInControls,
            allowsDebugMenu: allowsDebugMenu
        ) {
            EmptyView()
        }
    }
